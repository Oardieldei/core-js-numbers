...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const cos1 = x1 * x2 + y1 * y2;
  const cos2 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const cos3 = Math.sqrt(x2 ** 2 + y2 ** 2);
  const cosangle = cos1 / (cos2 * cos3);
...
function toNumber(value, def) {
  const newI = Number(value);
  return Number.isNaN(newI) ? def : newI;
}
